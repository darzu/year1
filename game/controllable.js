...
export function registerControllableSystems(em) {
    ...
    em.registerSystem([ControllableDef, LinearVelocityDef, RotationDef, WorldFrameDef], [InputsDef, PhysicsTimerDef, MeDef, CanvasDef], (controllables, res) => {
    ...
    em.registerSystem([ControllableDef, CameraFollowDef], [InputsDef, PhysicsTimerDef, MeDef, CanvasDef], (controllables, res) => {
        for (let c of controllables) {
            if (AuthorityDef.isOn(c) && c.authority.pid !== res.me.pid)
                continue;
            if (c.controllable.requiresPointerLock &&
