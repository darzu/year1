...
let __frame = 0;
export function initGJKSandbox(em, hosting) {
    ...
    em.registerOneShotSystem(null, [AssetsDef, GlobalCursor3dDef, RendererDef], (_, res) => {
        ...
        function createWorldShape(g, pos, rot, lastWorldPos) {
            ...
            const worldVerts = g.uniqueVerts.map((p) => vec3.transformMat4(tempVec(), p, transform));
...
export function initReboundSandbox(em, hosting) {
    const camera = em.addSingletonComponent(CameraDef);
    camera.fov = Math.PI * 0.5;
    let tableId = -1;
    em.registerOneShotSystem(null, [AssetsDef, GlobalCursor3dDef, RendererDef, TextDef], (_, res) => {
        const g = createGhost(em);
