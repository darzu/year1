...
import { getAABBFromMesh, getCenterFromAABB, getHalfsizeFromAABB, mapMeshPositions, scaleMesh, scaleMesh3, transformMesh, unshareProvokingVertices, } from "../render/mesh-pool.js";
...
const TETRA_MESH = unshareProvokingVertices(scaleMesh({
    pos: [
        [0, 1, 0],
        [-1, 0, -1],
        [1, 0, -1],
...
const HEX_MESH = () => {
    ...
    return unshareProvokingVertices(m2);
};
...
export function gameMeshFromMesh(mesh, renderer) {
...
function getUniqueVerts(mesh) {
    ...
    for (let v1 of mesh.pos) {
        if (!res.some((v2) => vec3.exactEquals(v1, v2)))
            res.push(v1);
    }
    return res;
}