...
export function registerCameraSystems(em) {
    ...
    em.registerSystem(null, [CameraViewDef, CameraDef, MeDef, CanvasDef], (_, resources) => {
        ...
        if (targetEnt) {
            const computedRotation = quat.mul(tempQuat(), frame.rotation, camera.targetRotationError);
            quat.normalize(computedRotation, computedRotation);
            const computedTranslation = vec3.add(tempVec(), frame.position, camera.targetPositionError);
            mat4.fromRotationTranslationScale(viewMatrix, computedRotation, computedTranslation, frame.scale);
        }
        ...
        if (camera.perspectiveMode === "ortho") {
            ...
            mat4.perspective(projectionMatrix, camera.fov, cameraView.aspectRatio, 1, 10000.0 /*view distance*/);
        }
