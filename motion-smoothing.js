...
export function registerMotionSmoothingSystems(em) {
    em.registerSystem([MotionSmoothingDef], [PhysicsTimerDef], (es, res) => {
        if (!res.physicsTimer.steps)
            return;
        const dt = res.physicsTimer.steps * res.physicsTimer.period;
        for (let e of es) {
            reduceError(e.motionSmoothing.positionError, dt, ERROR_SMOOTHING_FACTOR);
            reduceError(e.motionSmoothing.rotationError, dt, ERROR_SMOOTHING_FACTOR);
        }
    }, "smoothMotion");
    em.registerSystem([MotionSmoothingDef], [], (es) => {
        for (let e of es) {
